...
RSpec.describe StationListenerSyncJob, type: :job do
  ...
  describe "#perform" do
    context "when radio_stations feature is disabled" do
      ...
      it "returns early without querying stations or contacting Icecast" do
        playlist = create(:playlist, user: user)
        station = create(:radio_station, playlist: playlist, user: user, status: "active", listener_count: 5)

        StationListenerSyncJob.perform_now

        expect(station.reload.listener_count).to eq(5)
      end
    end
